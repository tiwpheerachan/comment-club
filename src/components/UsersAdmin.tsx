...
interface U { id: string; email: string; name: string; role: string; allowed_brands: string[]; active: boolean }
const ROLES = [
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-muted text-[11.5px] uppercase border-b border-line">
              ...
              <th className="text-left p-3 font-semibold">สิทธิ์ดูแบรนด์</th>
  ...
  return (
    <>
      <tr className="border-b border-[#eef0f3]">
        ...
        <td className="p-3 text-muted text-xs">{fullAccess ? "ทุกแบรนด์" : (u.allowed_brands || []).join(", ")}</td>
      ...
      {open && (
        <tr className="bg-slate-50/60"><td colSpan={5} className="p-4">
          ...
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            <button onClick={() => call("PATCH", { id: u.id, role, allowed_brands: sel }, "บันทึกแล้ว")} className="bg-shopee text-white px-3 py-1.5 rounded-lg text-xs font-semibold">บันทึก</button>
