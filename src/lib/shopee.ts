...
// trim กัน whitespace/newline ที่มักติดมาเวลาวางค่าใน Dashboard (ทำให้ fetch โยน "string did not match the expected pattern")
export const SHOPEE_REPLY_URL = (process.env.SHOPEE_REPLY_API_URL || DEFAULT_URL).trim();
export const SHOPEE_REPLY_KEY = (process.env.SHOPEE_REPLY_API_KEY || "").trim();
